...
export async function fetchBlogsPaginated(limit: number, offset: number): Promise<Blog> {
    const response =limit!=0? await fetch(`https://api.realsta.com/api/get_data.php?limit=${limit}&offset=${offset}`):await fetch(`https://api.realsta.com/api/get_data.php`);
    if (!response.ok) {
...
export async function fetchBlogsByCategory(limit: number, offset: number, catid: number): Promise<Blog> {
    const response = limit!=0? await fetch(`https://api.realsta.com/api/get_data.php?catid=${catid}&limit=${limit}&offset=${offset}`) : await fetch(`https://api.realsta.com/api/get_data.php?catid=${catid}`);
    if (!response.ok) {
        throw new Error('Failed to fetch blogs');
    }
    return response.json();
...
export async function fetchBlogByURL(postURL: string): Promise<IndividualBlog> {
    const response = await fetch(`https://api.realsta.com/api/get_data.php?postURL=${postURL}`);
    if (!response.ok) {
...
export async function fetchBlogsBySearch(text: string): Promise<Blog> {
    const response = await fetch(`https://api.realsta.com/api/get_data.php?q=${encodeURIComponent(text)}`);
    if (!response.ok) {
        throw new Error('Failed to fetch blogs');
    }
    return response.json();
